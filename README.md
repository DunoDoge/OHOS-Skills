...
三个 skill 互补：日常编码用 `arkts-helper`；命中 hvigor / ArkCompiler 报错时用 `arkts-debug`（未匹配的报错回退到 `arkts-helper` 的迁移规则文档）；写 `napi_*` / native 模块时用 `arkts-ndk-dev`。
...
链接完成后，在 opencode 内 skill 会按各自 `SKILL.md` 的 frontmatter `description` 自动触发：写 `.ets` / 提到 ArkUI、HarmonyOS NEXT、`arkts-no-*` 规则时加载 `arkts-helper`；粘贴编译错误时加载 `arkts-debug`；写 `napi_*` / 配置 CMake / 提到 OpenHarmony NDK 时加载 `arkts-ndk-dev`。
...
- 仓库整体（skill 元数据、`arkts-debug/`、`README.md`、`arkts-helper/SKILL.md`、`arkts-ndk-dev/SKILL.md` 等自行编写的内容）以 [MIT 许可](./LICENSE) 发布。
