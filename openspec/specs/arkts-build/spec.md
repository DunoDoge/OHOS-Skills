# arkts-build Specification

## Purpose

提供 `arkts-build` skill 的能力规范。该 skill 用于在 Agent 面向 HarmonyOS / OpenHarmony 工程执行「构建、依赖管理、静态检查、崩溃堆栈解析、测试、签名、设备部署、CI 流水线」类任务时，基于离线参考文档集给出准确的 HarmonyOS Command Line Tools 命令。
