# arkts-ndk-dev Specification

## Purpose

提供 `arkts-ndk-dev` skill 的能力规范。该 skill 用于在 Agent 面向 HarmonyOS / OpenHarmony 工程执行「NDK 开发 / Node-API 跨语言交互 / C++ 互操作 / CMake 工具链配置 / Native 资源管理 / NDK 调试性能分析 / 硬件兼容性」类任务时，基于离线参考文档集给出符合鸿蒙规范的 Node-API 代码与 NDK 配置。
